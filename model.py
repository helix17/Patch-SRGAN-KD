...
class PatchGANWithGlobalClassification(nn.Module):
    def __init__(
            self,
            in_channels: int = 3,
            out_channels: int = 1,
        ...
        super(PatchGANWithGlobalClassification, self).__init__()
        self.features = nn.Sequential(
            nn.Conv2d(in_channels, channels, kernel_size=4, stride=2, padding=1),  # Downsample
            ...
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(channels * 4, channels * 8, kernel_size=4, stride=1, padding=1),
            nn.BatchNorm2d(channels * 8),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(channels * 8, 1, kernel_size=4, stride=1, padding=1),  # Patch-level output
        )

        # Classification head for whole-image classification
        self.classifier = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),  # Global average pooling
            nn.Flatten(),            # Flatten the pooled tensor
            nn.Linear(1, 1),         # Output scalar value
            nn.Sigmoid()             # Use sigmoid for binary classification (real/fake)
        )

    def forward(self, x: Tensor) -> Tensor:
        patch_outputs = self.features(x)  # Patch-level discriminator output
        image_output = self.classifier(patch_outputs)  # Aggregate for whole-image classification
        return image_output
...
class ENetContentLoss(nn.Module):
    """
    Constructs a content loss function based on the EfficientNet network
    """
    ...
    def __init__(
        ...
        super(ENetContentLoss, self).__init__()

        model = getattr(models, net_cfg_name)(weights=models.EfficientNet_B0_Weights.IMAGENET1K_V1)
        # Extract the output of the feature extraction layer
        self.feature_extractor = create_feature_extractor(model, return_nodes=feature_nodes)
    ...
    def forward(self, sr_tensor: Tensor, gt_tensor: Tensor) -> torch.Tensor:
        """
        Forward pass for content loss calculation.

        Args:
            sr_tensor (torch.Tensor): Super-resolved tensor.
            gt_tensor (torch.Tensor): Ground truth tensor.

        Returns:
            torch.Tensor: Computed content loss as MSE between feature maps.
        """
        assert sr_tensor.size() == gt_tensor.size()
...
def discriminator_for_PatchGAN(**kwargs) -> PatchGANWithGlobalClassification:
    model = PatchGANWithGlobalClassification(**kwargs)
